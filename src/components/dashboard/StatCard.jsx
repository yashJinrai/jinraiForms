...
const StatCard = ({ title, value, icon, trend, color, breakdown, onClick }) => {
    ...
    return (
        <div 
            onClick={onClick}
            className={`bg-white p-6 rounded-[24px] border border-slate-50 shadow-sm hover:shadow-xl hover:shadow-[#3713ec]/5 transition-all duration-300 group ${onClick ? 'cursor-pointer' : ''}`}
        >
